...
class Solution {
    ...
    countBuildings(height) {
        ...
        if(n==1)return 1;
        // first building is tallest
        let tallest = height[0]; 
        // first building is always counted
        let count = 1;
        for(let i =1; i<n; i++){
            /// only count the building if it's tallest then the previous building
            if(height[i]>tallest){
                count++;
                // set the previous building as tallest
                tallest=height[i];
            }
        }
